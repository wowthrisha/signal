...
| R-05 | CUSUM floods alerts on volatile sessions | M | M | `h₂` too low; breadth suppression missing | Cut rule: D1 only if Gate 4 fails (Gate 4 = CUSUM stable at T+24); breadth guard (`breadth_t > 0.5` → one MARKET_REGIME card, suppress individual JUMPs). Replay harness now exists to measure the flood before shipping it (§13, Gate 2 PASS) | **OPEN** |
