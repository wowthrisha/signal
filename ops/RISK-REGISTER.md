...
| R-25 | Span-replacement edits silently delete unrelated content | M | M | Any edit of the form "replace everything between heading A and heading B" | **Fired 2026-09-05.** Regenerating the README's scaling section in [U6.3] replaced the whole span from `## How this scales` to `## What we deliberately did NOT build`, silently removing the `## What is actually new here` section added in [U4] — the GR-1 prior-art discussion. Not caught by any test, because prose has no test. Found by a heading listing while adding a new section; restored verbatim from `42030ee^` rather than rewritten from memory. Response: replace a named block, or append before an anchor — never a span whose contents are unread | **MITIGATED 2026-09-05** |
