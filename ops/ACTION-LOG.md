...
### FIX 4 — the first card no longer opens on load

**Chose: collapse it.** The reason the first card auto-opened was recorded in
the code — "so the page is not four closed boxes" — and that reason expired
when the funnel became the first block in this column. The top of the page is
now an open, populated graphic; the cards beneath it are a list to triage.

Measured at 1440×900, all cards collapsed:

| | |
|---|---|
| funnel | fully visible, 120–326px |
| first card | **fully visible**, 384–675px (291px tall) |
| second card | 210px of 291 visible |
| any drawer open on load | **no** — all four controls read "Investigate" |

The open card ran to 997px against a 901px fold, so the card a judge saw first
was cut through the middle of its outcome table. The funnel was not shrunk;
4a's requirement stands untouched.

The claim is now true as stated: **the funnel and one complete card fit above
the fold at 1440×900, and 72% of the second is visible.** Four cards still do
not fit and are not claimed to.

### FIX 5 — the Tailwind CDN warning

**Cannot be silenced without a build step. Left in place, documentation fixed.**

Two routes were checked rather than assumed:

  * **Overwrite `console.warn`.** Rejected. It hides a true statement about the
    deployment and every other warning with it.
  * **A prebuilt Tailwind stylesheet.** Would silence the warning with no build
    step — and would break the page. The markup uses **114 arbitrary-value
    utilities** (`text-[11px]` ×21, `text-[10px]` ×7, `tracking-[0.2em]` ×6,
    `tracking-[0.35em]`), and arbitrary values are generated by the JIT engine
    that only the play CDN ships. Every label's type scale and letter-spacing
    would fall back to browser defaults.

**The documentation was wrong, which the audit did not catch.** The README's
"What we deliberately did NOT build" row cited **ADR-033** for this decision.
ADR-033 is *One surface* — about not building a second page, not about the
build step. There was no ADR for the CDN at all.

**ADR-051** now records it: the warning, the count of arbitrary-value
