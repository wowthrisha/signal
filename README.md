...
434 passed, 2 skipped, 1 xfailed in 196.09s (0:03:16)
```
...
| A build step for the UI | The page loads Tailwind from its CDN, which prints a production warning we are choosing to accept rather than silence. The markup uses 114 arbitrary-value utilities (`text-[11px]`, `tracking-[0.2em]`), which only the CDN's JIT generates, so a prebuilt stylesheet is not a drop-in; and a bundler for one static page is infrastructure without demonstrated need. The full reasoning, including why the warning is not suppressed, is [ADR-051](docs/DECISIONS.md#adr-051-tailwinds-play-cdn-and-the-production-warning-it-prints). |
